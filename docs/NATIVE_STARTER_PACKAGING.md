...
The output has a shared `kernel` and eleven bundle directories: `parent`, `worker`, `runtime`
...
The catalogue exports `runtime`, whose only executable members are `/worker`
and `/pilot-fetch`. A scoped chat-only run therefore selects no borrowed tools
and carries none implicitly. The standing fleet worker remains a separate
bundle; old ten-bundle packages can still be admitted, but cannot provide this
isolated runtime without replacing the package.
