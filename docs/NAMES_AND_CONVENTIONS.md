...
| **warden** | Per-cell VMM. A thin, jailed userspace process that wraps and drives exactly one microVM (one cell). Holds the KVM fds, the narrow PIO broker channel, and sealed page maps; enforces the authority ratchet. Jailed so an escaped agent lands inside a process that has nothing. | host · unprivileged · one per cell |
